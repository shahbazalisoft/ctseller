                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row g-2">
                                ...
                                <div class="col-md-2 col-sm-2" id="stock_input">
                                ...
                                <div class="col-sm-2 col-lg-2" id="maximum_cart_quantity">
                                    <div class="form-group mb-0">
                                ...
                                <div class="col-md-4 col-sm-4" id="addon_input">
                                    <div class="form-group mb-0">
                                    ...
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="category_id" id="category_id" class="form-control js-select2-custom"
                                            ...
                                            @foreach($categories as $category)
                                                <option value="{{$category['id']}}" @if (json_decode($campaign->category_ids)[0]->id == $category['id']) selected @endif >{{$category['name']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="col-sm-6 col-lg-4">
                                    <div class="form-group mb-0">
                                        ...
                                        <select name="sub_category_id" id="sub-categories" class="form-control js-select2-custom">
                                            <?php $categories_two = isset(json_decode($campaign->category_ids)[1]->id) ? json_decode($campaign->category_ids)[1]->id : '';?>
                                            @foreach($newSubcateg as $subCat)
                                                <option value="{{$subCat['id']}}" @if ($categories_two == $subCat['id']) selected @endif >{{$subCat['name']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-12 col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <select id="category_featured" name="category_featured[]" class="form-control js-select2-custom" multiple="multiple">
                                            @foreach ($categories as $valCat)
                                                <option value="{{$valCat->id}}" @if($valCat->selected == 1) selected @endif>{{$valCat->name}}</option>
                                            @endforeach
                                    </select>
                                    </div>
                                </div>  
                                <div class="col-sm-6 col-lg-4">
                                    <div class="form-group mb-0">
                                        <label class="input-label"
                                            for="exampleFormControlInput1">{{ translate('messages.tax') }}
                                        </label>
                                        <select name="tax" id="tax"
                                            class="form-control js-select2-custom">
                                            <option value="7"  {{ $campaign['tax'] == '7' ? 'selected' : '' }} >{{  translate('messages.7_%') }}</option>
                                            <option value="19" {{ $campaign['tax'] == '19' ? 'selected' : '' }}>{{ translate('messages.19_%') }}</option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="row g-2">
                                ...
                                <div class="col-md-3 col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label"
                                            for="exampleFormControlInput1">{{ translate('messages.net_price') }} </label>
                                        <input type="number" value="{{ $campaign->price }}" min="0"
                                            max="100000" name="price" class="form-control" step="0.01" 
                                            placeholder="{{ translate('messages.Ex:') }} 100" required id="nettoPrice">
                                    </div>
                                </div>
                                <div class="col-md-3 col-sm-6">
                                    <div class="form-group mb-0">
                                        <label class="input-label"
                                            for="exampleFormControlInput1">{{ translate('messages.gross_price') }} </label>
                                        <input type="number" value="{{ $campaign->brutto_price }}" min="0"
                                            max="100000" name="brutto_price" class="form-control" step="0.01"
                                            placeholder="{{ translate('messages.Ex:') }} 100" required id="bruttoPrice">
                                    </div>
                                </div>
                                <div class="col-md-3 col-sm-6">
                                    <div class="form-group mb-0">
                            ...
                            <div class="row g-2">
                                ...
                                <div class="col-md-12">
                                    <div class="variant_combination" id="variant_combination">
                                        @include('vendor-views.campaign.partials.edit-combination',['combinations'=>json_decode($campaign['variations'],true),'stock'=>config('module.'.$campaign->module->module_type)['stock']])
                                    </div>
                                </div>
                            </div>
                        </div>
...
@push('script_2')
    ...
    <script>
        ...
        function combination_update() {
            ...
            $.ajax({
                type: "POST",
                url: "{{route('vendor.campaign.variant-combination')}}",
    ...
    <script>
        ...
        $('#campaign_form').on('submit', function(e) {
            ...
            $.post({
                url: '{{route('vendor.campaign.update-item', [$campaign->id])}}',
                data: $('#campaign_form').serialize(),
                data: formData,
                cache: false,
                contentType: false,
                ...
                success: function(data) {
                    ...
                    if (data.errors) {
                        ...
                        setTimeout(function() {
                            location.href =
                                '{{ route('vendor.campaign.itemlist') }}';
                        }, 2000);
                    }
                }
            });
    ...
    $(document).ready(function() {
        ...
        $(document).ready(function() {
        // Event listener for tax change
        $('#tax').change(function() {
            var tax = parseFloat($(this).val());
            var nettoPrice = parseFloat($('#nettoPrice').val());

            if (!isNaN(nettoPrice)) {
                var bruttoPrice = nettoPrice * (1 + (tax / 100));
                $('#bruttoPrice').val(formatPrice(bruttoPrice));
            }
        });
        ...
        $('#bruttoPrice').change(function() {
            ...
            if (!isNaN(bruttoPrice)) {
                var nettoPrice = bruttoPrice / (1 + (tax / 100));
                $('#nettoPrice').val(formatPrice(nettoPrice));
            }
        });
    });
