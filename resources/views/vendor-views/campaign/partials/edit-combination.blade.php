    ...
    <table class="table table-borderless table--vertical-middle">
        ...
        @foreach ($combinations as $key => $combination)
            <tr>
                ...
                <td>
                    <input type="number" name="brutto_price_{{ $combination['type'] }}"
                           value="{{isset($combination['brutto_price'])?$combination['brutto_price'] : 0}}" min="0"
                           step="0.01"
                           class="form-control bruttoPrice" required>
                </td>
                {{-- @if ($stock) --}}
                    <td>
